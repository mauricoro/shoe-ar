...
//    var isOptional = false
    ...
    //variable for camera permission
    // can be either tru or false
    var onCameraPermissionResult: (isGranted: Boolean) -> Unit = { isGranted ->
        // if granted, proceed with the activity
        if (!isGranted) {
            if (!ActivityCompat.shouldShowRequestPermissionRationale(
                    activity,
                    Manifest.permission.CAMERA
                )
                //otherwise, open the application settings to manually enable
                //camera permissions
            ) {
    ...
    //Thiss function takes the user to the app settings in order to turn on camera permissions
    fun showCameraPermissionSettings(activity: Activity) {
        // Permission denied with checking "Do not ask again".
        // Output a message at the bottom of the screen saying that camera permission is required
        Toast.makeText(
            activity,
    ...
    // Check if AR core is available for the phone
